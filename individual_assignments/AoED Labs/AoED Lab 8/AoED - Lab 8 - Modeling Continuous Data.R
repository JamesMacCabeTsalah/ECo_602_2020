...
# t-tests are useful when we have a categorical predictor with two levels and a continuous response variabl
...
# we are using bootstrapping to characterize the sampling distribution of the alternative hypothesis! ***
pen_boot = two.boot(
...
# Q7.1: 4 & 29.125 
# Q7.2: 16 is in between 4 - 29.125
...
# when doing CI's with bootstrapping, we are doing a two sided, not one sided! *****
  # CI bootstrapping is to tell us how precise we feel about our measurements - and thus mean of measurements - and a one sided measurement doesn't tell us anything
...

{
  ...
  m = # input number of resamples you want
  result = numeric(m) # we can pre-allocate a vector to hold the results using numeric()
...
# ---- Q10 ----
quantile(result, 0.05)
# Q10.1.1: Critical Value = p-value = -0.0132565
# Q10.1.2: The observed slope of -0.0244 was smaller than the critical value of -0.0132565

# Q10.2: My conclusion is that because the observed slope is below the critical value, we can reject the null distribution.
# This means that the negative relationship between vegetation cover diversity and bird diversity is unlikely to be 
# observed due to random chance! 

# we are doing a one tailed because its a negative slope - and one tailed only computes 1 CI value
  # two tailed computes two CI values and would be for example 2.25 and 97.75 CI's
    # this is you allocating the alpha = 0.05 to two sides!

# one tailed tests are more powerful than two tailed because you need to know more to make a stronger inference to make a one tailed test
  # to make a one tailed test you have to specify what we expect as an output, and rule out a side in doing so to make it one tailed
  # an observed value can be rejected in a two tailed hypothesis, but be accepted in a one tailed! ***

# if the slope is above the critical, the slope is in the rejection region where the data is likely to occur if the null hypothesis were true
# if the slope is below the critical value, this rejects the null distribution 
